...
class DAL {
  ...
  //////////////////////////////////////////////sparepart start here///////////////////////////////////////////////
  public static function addSparePart($request) {
    global $connection;
    ...
    $requiredField = array("Date"=>"$sparePartLogDate", "Brand"=>"$brandID", "Model"=>"$modelID", "Serial"=>"$serialName", "Check-in Store"=>"$locationID", "Type"=>"$sparePartTypeID");
    foreach ($requiredField as $field => $value) {
      if ($value == '') {    
        echo "<div class='alert alert-warning' role='alert'>";
        echo "{$field} is empty.<br>";
    ...
    if (empty($remarksContent)) {
      ...
      $sql2 = "INSERT INTO `sparepart`(`brandID`, `modelID`, `locationID`,`sparePartTypeID`, `serialID`, `statusID`) ";
      $sql2 .= "VALUES ('{$brandID}', '{$modelID}', '{$locationID}','{$sparePartTypeID}', '{$serialID}', 1)";
      $sql2 = mysqli_query($connection, $sql2);
      if (!$sql2) {
        echo DAL::ErrorNo();
      }
      $sparePartID = mysqli_insert_id($connection); //get sparepartID
      ...
      $sql2 = "INSERT INTO `sparepart`(`brandID`, `modelID`, `locationID`, `serialID`, `statusID`, `sparePartTypeID`) ";
      $sql2 .= "VALUES ('{$brandID}', '{$modelID}', '{$locationID}', '{$serialID}', '{$sparePartTypeID}', 1)";
      $sql2 = mysqli_query($connection, $sql2);
      if (!$sql2) {
        DAL::Error();
      }
      ...
      $sparePartLogID = mysqli_insert_id($connection); //get sparePartLogID
  ...
  public static function updateSparePartCheckoutDetails($request, $sparePartID) {
    global $connection;
    $checkoutDate = $_POST['checkoutDate'];
    $checkoutLocation = $_POST['checkoutLocation'];
    $checkoutRemarks = $_POST['checkoutRemarks'];

    $sql = "SELECT `sparepartlog`.`sparePartLogID`, `sparepartlog`.`txnTypeID`, `sparepartlog`.`sparePartID` ";
    $sql .= "FROM `sparepartlog` ";
    $sql .= "WHERE `sparepartlog`.`txnTypeID` = 3 AND `sparepartlog`.`sparePartID` = {$sparePartID}";
    $sql = mysqli_query($connection, $sql);
    if (mysqli_num_rows($sql) > 0) {
      $sparePartHasCheckoutData = true;
    } else {
      $sparePartHasCheckoutData = false;
    }
    if (!$sparePartHasCheckoutData && empty($checkoutRemarks)) { // if no database result & checkoutRemarks field is empty
      mysqli_begin_transaction($connection);
      $sql1 = "INSERT INTO `sparepartlog`(`sparePartLogDate`, `locationID`, `sparePartID`, `txnTypeID`, `userID`) ";
      $sql1 .= "VALUES ('{$checkoutDate}', '{$checkoutLocation}', '{$sparePartID}', 3, 1)";
      $sql1 = mysqli_query($connection, $sql1);
        if (!$sql) {
          DAL::Error();
        }
      $sql2 = "UPDATE `sparepart` SET `locationID` = {$checkoutLocation}, `statusID` = 2 ";
      $sql2 .= "WHERE `sparePartID` = {$sparePartID}";
      $sql2 = mysqli_query($connection, $sql2);
      if (!$sql2) {
        DAL::Error();
      }
    ...
    } elseif (!$sparePartHasCheckoutData && !empty($checkoutRemarks)) { //if no database result & checkoutRemarks is not empty
      mysqli_begin_transaction($connection);
      $sql1 = "INSERT INTO `sparepartlog`(`sparepartLogDate`, `locationID`, `sparePartID`, `txnTypeID`, `userID`) ";
      $sql1 .= "VALUES ('{$checkoutDate}', '{$checkoutLocation}', '{$sparePartID}', 3, 1)";
      $sql1 = mysqli_query($connection, $sql1);
      ...
      $sql3 = "INSERT INTO `sparepartlog_has_remarks`(`sparePartLogID`, `remarksID`) VALUES ('{$sparePartLogID}', '{$remarksID}')";
      $sql3 = mysqli_query($connection, $sql3);
      if (!$sql3) {
        DAL::Error();
      }

      $sql4 = "UPDATE `sparepart` SET `locationID` = {$checkoutLocation}, `statusID` = 2 ";
      $sql4 .= "WHERE `sparePartID` = {$sparePartID}";
      $sql4 = mysqli_query($connection, $sql4);
      if (!$sql4) {
        DAL::Error();
      }
    ...
    } elseif ($sparePartHasCheckoutData && empty($checkoutRemarks)) { //if exists database result & checkoutRemarks is empty
      mysqli_begin_transaction($connection);
      $sql1 = "UPDATE `sparepartlog` SET `sparePartLogDate` = '{$checkoutDate}', `locationID` = '{$checkoutLocation}' ";
    ...
    } else { //check for $sparePartHasCheckoutData && empty($checkoutRemarks)
      $sql1 = "SELECT `sparepartlog`.`sparePartLogID`, `sparepartlog`.`sparePartID`, `sparepartlog`.`txnTypeID` ";
      $sql1 .= "FROM `sparepartlog` ";
      $sql1 .= "WHERE `sparepartlog`.`txnTypeID` = 3 AND `sparepartlog`.`sparePartID` = {$sparePartID}";
      $sql1 = mysqli_query($connection, $sql1);
      while ($row = mysqli_fetch_assoc($sql1)) {
        $sparePartLogID = $row['sparePartLogID'];
      }

      $sql2 = "SELECT `remarks`.`remarksContent`, `sparepartlog`.`sparePartID`, `sparepartlog`.`txnTypeID` ";
      $sql2 .= "FROM `remarks` ";
      $sql2 .= "LEFT JOIN `sparepartlog` ON `remarks`.`sparePartLogID` = `sparepartlog`.`sparePartLogID` ";
      $sql2 .= "WHERE `sparepartlog`.`sparePartID`= {$sparePartID} AND `sparepartlog`.`txnTypeID` = 3";
      $sql2 = mysqli_query($connection, $sql2);
      if (mysqli_num_rows($sql2) > 0) {
        $sparePartHascheckoutRemarks = true;
      } else {
        $sparePartHascheckoutRemarks = false;
      }

      if ($sparePartHascheckoutRemarks == true) {
        mysqli_begin_transaction($connection);
        $sql3 = "UPDATE `sparepartlog` SET `sparePartLogDate` = '{$checkoutDate}', `locationID` = '{$checkoutLocation}' ";
        $sql3 .= "WHERE `sparepartlog`.`sparePartID` = {$sparePartID} AND `sparepartlog`.txnTypeID = 3";
        $sql3 = mysqli_query($connection, $sql3);
        if (!$sql3) {
          DAL::Error();
        }
        ...
        $sql5 = "UPDATE `remarks` SET `remarksContent` = '{$checkoutRemarks}' ";
        $sql5 .= "WHERE `sparePartLogID` = {$sparePartLogID}";
        $sql5 = mysqli_query($connection, $sql5);
        if (!$sql5) {
          DAL::Error();
        }
        ...
        $sql5 = "INSERT INTO `remarks`(`sparePartLogID`, `remarksContent`) ";
        $sql5 .= "VALUES ('{$sparePartLogID}', '{$checkoutRemarks}')";
        $sql5 = mysqli_query($connection, $sql5);
        if (!$sql5) {
          DAL::Error();
        }
  ...
  public static function updateSparePartTransferDetails($request, $sparePartID) {
    global $connection;
    $transferDate = $request['transferDate'];
    $transferLocation = $request['transferLocation'];
    $transferRemarks = $request['transferRemarks'];
    ...
    $sql = "SELECT `sparepartlog`.`sparePartLogID`, `sparepartlog`.`txnTypeID`, `sparepartlog`.`sparePartID` ";
    $sql .= "FROM `sparepartlog` ";
    $sql .= "WHERE `sparepartlog`.`txnTypeID` = 2 AND `sparepartlog`.`sparePartID` = {$sparePartID}";
    $sql = mysqli_query($connection, $sql);
    if (mysqli_num_rows($sql) > 0) {
      $sparePartHasTransferData = true;
    } else {
      $sparePartHasTransferData = false;
    }
    if (!$sparePartHasTransferData && empty($transferRemarks)) { // if no database result & transferRemarks field is empty
      mysqli_begin_transaction($connection);
      $sql1 = "INSERT INTO `sparepartlog`(`sparePartLogDate`, `locationID`, `sparePartID`, `txnTypeID`, `userID`) ";
      $sql1 .= "VALUES ('{$transferDate}', '{$transferLocation}', '{$sparePartID}', 2, 1)";
      $sql1 = mysqli_query($connection, $sql1);
        if (!$sql) {
          DAL::Error();
        }
      $sql2 = "UPDATE `sparepart` SET `locationID` = {$transferLocation} ";
      $sql2 .= "WHERE `sparePartID` = {$sparePartID}";
      $sql2 = mysqli_query($connection, $sql2);
      if (!$sql2) {
        DAL::Error();
    ...
    } elseif (!$sparePartHasTransferData && !empty($transferRemarks)) { //if no database result & transferRemarks is not empty
      mysqli_begin_transaction($connection);
      $sql1 = "INSERT INTO `sparepartlog`(`sparePartLogDate`, `locationID`, `sparePartID`, `txnTypeID`, `userID`) ";
      $sql1 .= "VALUES ('{$transferDate}', '{$transferLocation}', '{$sparePartID}', 2, 1)";
      $sql1 = mysqli_query($connection, $sql1);
      ...
      $sql4 = "UPDATE `sparepart` SET `locationID` = {$transferLocation} ";
      $sql4 .= "WHERE `sparePartID` = {$sparePartID}";
      $sql4 = mysqli_query($connection, $sql4);
      if (!$sql4) {
        DAL::Error();
    ...
    } elseif ($sparePartHasTransferData && empty($transferRemarks)) { //if exists database result & transferRemarks are empty
      mysqli_begin_transaction($connection);
      $sql1 = "UPDATE `sparepartlog` SET `sparePartLogDate` = '{$transferDate}', `locationID` = '{$transferLocation}' ";
      ...
      $sql3 = "SELECT `sparepartlog`.`sparePartLogID`, `sparepartlog`.`sparePartID`, `sparepartlog`.`txnTypeID`, `remarks`.`remarksID`, `remarks`.`remarksContent` ";
      $sql3 .= "FROM `sparepartlog` ";
      $sql3 .= "JOIN `sparepartlog_has_remarks` ON `sparepartlog_has_remarks`.`sparePartLogID` = `sparepartlog`.`sparePartLogID` ";
      $sql3 .= "JOIN `remarks` ON `sparepartlog_has_remarks`.`remarksID` = `remarks`.`remarksID` ";
      $sql3 .= "WHERE (`sparepartlog`.`sparePartID` = {$sparePartID}) ";
      $sql3 .= "AND   (`sparepartlog`.`txnTypeID` = 2)";
      $sql3 = mysqli_query($connection, $sql3);
      while ($row = mysqli_fetch_assoc($sql3)) {
        $remarksID = $row['remarksID'];
      }
      ...
      $sql1 = "SELECT `sparepartlog`.`sparePartLogID`, `sparepartlog`.`sparePartID`, `sparepartlog`.`txnTypeID` ";
      $sql1 .= "FROM `sparepartlog` ";
      $sql1 .= "WHERE `sparepartlog`.`txnTypeID` = 2 AND `sparepartlog`.`sparePartID` = {$sparePartID}";
      $sql1 = mysqli_query($connection, $sql1);
      while ($row = mysqli_fetch_assoc($sql1)) {
        $sparePartLogID = $row['sparePartLogID'];
      }

      $sql2 = "SELECT `sparepartlog`.`sparePartLogID`, `sparepartlog`.`sparePartID`, `sparepartlog`.`txnTypeID`, `remarks`.`remarksID`, `remarks`.`remarksContent` ";
      $sql2 .= "FROM `sparepartlog` ";
      $sql2 .= "JOIN `sparepartlog_has_remarks` ON `sparepartlog_has_remarks`.`sparePartLogID` = `sparepartlog`.`sparePartLogID` ";
      $sql2 .= "JOIN `remarks` ON `sparepartlog_has_remarks`.`remarksID` = `remarks`.`remarksID` ";
      $sql2 .= "WHERE (`sparepartlog`.`sparePartID` = {$sparePartID}) ";
      $sql2 .= "AND   (`sparepartlog`.`txnTypeID` = 2)";
      $sql2 = mysqli_query($connection, $sql2);
      if (mysqli_num_rows($sql2) > 0) {
        $sparePartHasTransferRemarks = true;
      } else {
        $sparePartHasTransferRemarks = false;
      }

      if ($sparePartHasTransferRemarks == true) {
        mysqli_begin_transaction($connection);
        $sql3 = "UPDATE `sparepartlog` SET `sparePartLogDate` = '{$transferDate}', `locationID` = '{$transferLocation}' ";
        $sql3 .= "WHERE `sparepartlog`.`sparePartID` = {$sparePartID} AND `sparepartlog`.txnTypeID = 2";
        $sql3 = mysqli_query($connection, $sql3);
        if (!$sql3) {
          DAL::Error();
        }
        ...
        //search for corresponding remarksID by sparepart's logID
        $sql5 = "SELECT `sparepartlog`.`sparePartID`, `sparepartlog`.`sparePartLogID`, `sparepartlog`.`txnTypeID`, `remarks`.`remarksID` ";
        $sql5 .= "FROM `sparepartlog` ";
        $sql5 .= "JOIN `sparepartlog_has_remarks` ON `sparepartlog_has_remarks`.`sparePartLogID` = `sparepartlog`.`sparePartLogID` ";
        $sql5 .= "JOIN `remarks` ON `sparepartlog_has_remarks`.`remarksID` = `remarks`.`remarksID` ";
        $sql5 .= "WHERE (`sparepartlog`.`sparePartID` = {$sparePartID}) ";
        $sql5 .= "AND   (`sparepartlog`.`txnTypeID` = 2)";
        $sql5 = mysqli_query($connection, $sql5);
        while ($row = mysqli_fetch_assoc($sql5)) {
          $remarksID = $row['remarksID'];
        }
        ...
        mysqli_begin_transaction($connection);
        $sql6 = "UPDATE `sparepartlog` SET `sparePartLogDate` = '{$transferDate}', `locationID` = '{$transferLocation}' ";
        $sql6 .= "WHERE `sparepartlog`.`sparePartID` = {$sparePartID} AND `sparepartlog`.txnTypeID = 2";
        $sql6 = mysqli_query($connection, $sql6);
        if (!$sql6) {
          DAL::Error();
        }

        $sql7 = "UPDATE `sparepart` SET `locationID` = {$transferLocation} ";
        $sql7 .= "WHERE `sparePartID` = {$sparePartID}";
        $sql7 = mysqli_query($connection, $sql7);
        if (!$sql7) {
          DAL::Error();
        }
        ...
        $sql10 = "INSERT INTO `sparepartlog_has_remarks`(`sparePartLogID`, `remarksID`) VALUES ('{$sparePartLogID}', '{$remarksID}')";
        $sql10 = mysqli_query($connection, $sql10);
        if (!$sql10) {
          DAL::Error();
        }
  ...
  public static function updateSparePartCheckinDetails($request, $sparePartID) {
    global $connection;
    $checkinDate = $request['checkinDate'];
    $checkinLocation = $request['checkinLocation'];
    $checkinRemarks = $request['checkinRemarks'];

    $sql = "SELECT `sparepartlog`.`sparePartLogID`, `sparepartlog`.`sparePartID`, `sparepartlog`.`txnTypeID`, `remarks`.`remarksID`, `remarks`.`remarksContent` ";
    $sql .= "FROM `sparepartlog` ";
    $sql .= "JOIN `sparepartlog_has_remarks` ON `sparepartlog_has_remarks`.`sparePartLogID` = `sparepartlog`.`sparePartLogID` ";
    $sql .= "JOIN `remarks` ON `sparepartlog_has_remarks`.`remarksID` = `remarks`.`remarksID` ";
    $sql .= "WHERE (`sparepartlog`.`sparePartID` = {$sparePartID}) ";
    $sql .= "AND   (`sparepartlog`.`txnTypeID` = 1)";
    $sql = mysqli_query($connection, $sql);
    if (mysqli_num_rows($sql) > 0) {
      $sparePartHasCheckinRemarks = true;
    } else {
      $sparePartHasCheckinRemarks = false;
    }

    $sql2 = "SELECT `sparepartlog`.`sparePartLogID`, `sparepart`.`sparePartID`, `sparepartlog`.`txnTypeID` ";
    $sql2 .= "FROM `sparepartlog` ";
    $sql2 .= "JOIN `sparepart` ON `sparepartlog`.`sparePartID` = `sparepart`.`sparePartID` ";
    $sql2 .= "WHERE `sparepart`.`sparePartID` = {$sparePartID} AND `sparepartlog`.`txnTypeID` = 1";
    $sql2 = mysqli_query($connection, $sql2);
    if (mysqli_num_rows($sql2) > 0) {
      while ($row = mysqli_fetch_assoc($sql2)) {
        $sparePartLogID = $row['sparePartLogID'];
    ...
    if (($sparePartHasCheckinRemarks == false && empty($checkinRemarks))) {
      $sql1 = "UPDATE `sparepartlog` SET `sparePartLogDate` = '{$checkinDate}', `locationID` = {$checkinLocation} ";
      $sql1 .= "WHERE `sparePartID` = {$sparePartID} AND  `txnTypeID` = 1";
      $sql1 = mysqli_query($connection, $sql1);
      if (!$sql1) {
        die('Failed. '.mysqli_error($connection));
      }
    } elseif ($sparePartHasCheckinRemarks == true && !empty($checkinRemarks)) {
      //fetch sparepartlog ada remarksid 

      $sql2 = "SELECT `sparepartlog`.`sparePartID`, `sparepartlog`.`sparePartLogID`, `sparepartlog`.`txnTypeID`, `remarks`.`remarksID` ";
      $sql2 .= "FROM `sparepartlog` ";
      $sql2 .= "JOIN `sparepartlog_has_remarks` ON `sparepartlog_has_remarks`.`sparePartLogID` = `sparepartlog`.`sparePartLogID` ";
      $sql2 .= "JOIN `remarks` ON `sparepartlog_has_remarks`.`remarksID` = `remarks`.`remarksID` ";
      $sql2 .= "WHERE (`sparepartlog`.`sparePartID` = {$sparePartID}) ";
      $sql2 .= "AND   (`sparepartlog`.`txnTypeID` = 1)";
      $sql2 = mysqli_query($connection, $sql2);
      while ($row = mysqli_fetch_assoc($sql2)) {
        $remarksID = $row['remarksID'];
      }
      ...
      $sql2 = "INSERT INTO `sparepartlog_has_remarks`(`sparePartLogID`, `remarksID`) VALUES ('{$sparePartLogID}', '{$remarksID}')";
      $sql2 = mysqli_query($connection, $sql2);
      if (!$sql2) {
        DAL::Error();
      }
